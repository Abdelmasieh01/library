...
def search(request):
    ...
    books_author = Book.objects.filter(Q(author__icontains=keyword)).order_by('category', 'code')
    context = {'books': books, 'books_author': books_author, 'keyword': keyword}
...
def create_borrowing(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if book.copies > 0:
                borrowing = Borrowing.objects.create(borrower=borrower, borrow_date=date, book=book)
                borrowing.borrow()
...
def return_book(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            borrowing = Borrowing.objects.get(pk=request.POST.get('borrowing', ''))
